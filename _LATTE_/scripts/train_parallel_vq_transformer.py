...
from src.data.datasets.vq_token_parallel import collate_parallel_vq_tokens
...
class ParallelVQTransformer(nn.Module):
    """
    Parallel next-frame VQ-token predictor.
    ...
    def forward(
        ...
        channel_id: Optional[torch.Tensor] = None,
    ):
        """
        ...
        if self.channel_emb is not None and channel_id is not None:
            ch = channel_id.clamp(0, self.max_channels - 1)
            x = x + self.channel_emb(ch).unsqueeze(1)

        x = self.drop(x)

        # nn.TransformerEncoder expects True where positions should be ignored.
...
def main():
    ...
    ds_tr = build_dataset_from_registry(reg, name=args.dataset_name, split="train", extra_kwargs=extra_train)
    ds_va = build_dataset_from_registry(reg, name=args.dataset_name, split="val", extra_kwargs=extra_val)

    pad_id = int(ds_tr.pad_id)

    collate_fn = partial(collate_parallel_vq_tokens, pad_id=pad_id)
    ...
    while step < args.max_steps:
        for batch in dl_tr:
            ...
            if (step % args.eval_every) == 0:
                ...
                print(
                    f"[{step:>7}] lr={lr:.3e} train_loss={float(loss.item()):.4f} "
                ...
                extra = dict(
                    ...
                    model_type="parallel_vq_next_frame",
                )
